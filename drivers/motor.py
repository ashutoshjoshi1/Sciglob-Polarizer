...
class MotorConnectThread(QThread):
    """Thread to attempt motor serial connection with baud and parity auto-detection."""
    result_signal = pyqtSignal(object, int, str)  # emits (serial_obj or None, baud_rate, message)
    ...
    def run(self):
        ...
        found_baud = None
        message = ""
        # Try each baud rate and parity to find a responding motor
        ...
        for baud in BaudRateList:
            for parity in parities:
                try:
                    ...
                    # Test Modbus read (function 0x03) at register 0x0058 (2 registers)
                    ...
                    if response:
                        ...
                        found_baud = baud
                        message = (
                            f"Motor connected on {self.port_name} at {baud} baud, parity={parity}."
                        )
                        break
                    ser.close()
                except Exception:
        ...
        # Emit result
        self.result_signal.emit(found_serial, found_baud or 0, message)


def send_move_command(serial_obj, angle: int) -> bool:
    """Send a move command to the motor to go to the specified angle (degrees)."""
    # Construct Modbus Write Multiple Registers command
    base_cmd = bytes([
        SlaveID, 0x10, 0x00, 0x58, 0x00, 0x12, 0x24,
    ...
    try:
        ...
        serial_obj.write(full_cmd + crc_val.to_bytes(2, 'little'))
        response = serial_obj.read(8)
        return bool(response and len(response) >= 6 and response[1] == 0x10)
    except Exception:
        return False
